import { LineChart, Line, YAxis, Tooltip, ResponsiveContainer, TooltipProps } from 'recharts';
...
const TotalScore = ({ recentPracticeData }: { recentPracticeData: RecentFeedback }) => {
  ...
  return (
    <div className='white-card col-span-3 col-start-9 row-span-2 row-start-2 bg-linear-to-r from-[rgba(76,154,207,1)] via-[rgba(120,192,210,1)] to-[rgba(169,234,214,1)]'>
      <p className='c1 text-gray-50'>{recentPracticeData.practiceCount}회차 연습 결과</p>
      <div className='flex w-full items-baseline justify-between text-white'>
        <span className='h1'>{Math.round(recentPracticeData.graph.currentScore)}점</span>
      </div>
